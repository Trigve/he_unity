...
#region Enums
	...
	public bool CheckDirection(Direction Dir, float Ratio)
	{
		...
		he.Map map = terrain_manager.map;
		he.TerrainTile last_tile = map.GetTile(map.width - 1, map.height - 1);
		...
		{
			case Direction.LEFT:
				ret = point_on_plane.z - amount_ratio > he.TerrainPartition.TILE_WIDTH;
				break;
			case Direction.TOP:
				ret = point_on_plane.x - amount_ratio > he.TerrainPartition.TILE_HEIGHT;
				break;
			case Direction.BOTTOM:
	...
	private void RecalculateCamera()
	{
		...
		Vector3 point_on_plane = ray.GetPoint(point);
		// Normalize position
		transform.Translate(0, 0, -point_on_plane.z + he.TerrainPartition.TILE_WIDTH, Space.World);
		transform.Translate(point_on_plane.x + he.TerrainPartition.TILE_HEIGHT, 0, 0, Space.World);
	}
