...
#region Fields
		...
		//! All tiles.
		protected he.TerrainTile[] m_Tiles;
		//! Terrain tile properties.
		protected Dictionary<ushort, he.TerrainTileProperty> m_Properties;
...
#region Interface
		//! Get tile.
		public he.TerrainTile GetTile(int X, int Y)
		{
			return m_Tiles[GetTileIndex(X, Y)];
		}

		public he.TerrainTile GetTile(int Triangle)
		{
			return m_Tiles[Triangle / 2];
		}

		//! Get tile for given triangle.
		public he.TerrainTileHandle GetTileHandle(int Triangle)
		{
			he.TerrainTile tile = GetTile(Triangle);

			return new he.TerrainTileHandle(tile.x, tile.y, m_PositionX, m_PositionY);
		}
		...
		*/
		...
		public Mesh CreateMesh(he.TerrainTileSet TileSet)
		{
			// Create vertex array
			...
			for (int i = 0, last_vertex = 0; i < PARTITION_HEIGHT; ++i)
			{
				for (int j = 0; j < PARTITION_WIDTH; ++j)
				{
					...
					// Get Tile
					he.TerrainTile tile = m_Tiles[tile_index];
					// Create vertices
					...
					byte mat_v0 = tile.GetTerrainType(he.TerrainTile.Vertex.NORTH),
						mat_v1 = tile.GetTerrainType(he.TerrainTile.Vertex.WEST),
						mat_v2 = tile.GetTerrainType(he.TerrainTile.Vertex.SOUTH),
						mat_v3 = tile.GetTerrainType(he.TerrainTile.Vertex.EAST);
					...
					he.TextureAtlasInfo primary_mat_info = TileSet.GetTileType(mat_1, tile.variant);
					// Get secondary tile type information
					he.TextureAtlasInfo secondary_mat_info = TileSet.GetTileType(mat_2, tile.variant);
					// Get the alpha splat info for tile
					he.TextureAtlasInfo alpha_splat_mat_info = TileSet.splatUsed.GetSplat(alpha_index);
...
#region Operations
		...
		private he.TerrainTileProperty GetCreateTileProperty(int X, int Y)
		{
			ushort tile_index = (ushort)GetTileIndex(X, Y);
			if (!m_Properties.ContainsKey(tile_index))
				m_Properties[tile_index] = new he.TerrainTileProperty();
...
#region Construction
		public void This(int PartitionX, int PartitionY, he.TerrainTileSet TileSet)
		{
			m_PositionX = PartitionX;
			m_PositionY = PartitionY;
			// Create tiles
			m_Tiles = new he.TerrainTile[PARTITION_WIDTH * PARTITION_HEIGHT];
			...
			for (int j = 0; j < PARTITION_HEIGHT; ++j)
			{
				for (int i = 0; i < PARTITION_WIDTH; ++i)
				{
					...
					// Set random variant
					he.TerrainTile.Type tile_type = he.TerrainTile.Type.REGULAR;
					he.TerrainTile tile = new he.TerrainTile(i, j, tile_type, (byte)rnd.Next(0, 7));
//					tile.variant = 0;
